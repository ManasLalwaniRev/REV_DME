...
import React, { useState, useEffect } from 'react';
import bcrypt from 'bcryptjs';
import { Save, UserPlus, Users, PlusCircle, Trash2, ArrowLeft, Loader, LogOut, UserCircle, Settings } from 'lucide-react';
...
const SettingsAndProfilePage = ({ setCurrentPage, currentUserId, currentUsername, currentUserRole, handleLogOut}) => {
  ...
  // Now managed locally to avoid API errors
  const [contractOptions, setContractOptions] = useState([
    { id: 1, name: 'Standard Prime' },
    { id: 2, name: 'Sub-Contract A' }
  ]);
  const [creditCardOptions, setCreditCardOptions] = useState([
    { id: 1, name: 'Corporate Amex' },
    { id: 2, name: 'Visa Gold' }
  ]);
  ...
  // --- Local Handlers for Options (No API) ---
  const handleAddLocalOption = (type) => {
    if (type === 'contract' && newContract) {
      setContractOptions([...contractOptions, { id: Date.now(), name: newContract }]);
      setNewContract('');
      setMessage('Contract added locally.');
    } else if (type === 'card' && newCard) {
      setCreditCardOptions([...creditCardOptions, { id: Date.now(), name: newCard }]);
      setNewCard('');
      setMessage('Card added locally.');
    }
  };

  const handleDeleteLocalOption = (id, type) => {
    if (type === 'contract') {
      setContractOptions(contractOptions.filter(opt => opt.id !== id));
    } else {
      setCreditCardOptions(creditCardOptions.filter(opt => opt.id !== id));
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black p-4 lg:p-8 text-gray-100 flex justify-center items-start font-sans">
      <div className="bg-white p-6 sm:p-10 rounded-xl shadow-2xl w-[98%] max-w-full text-gray-800">
        ...
        {/* --- Standardized Lumina Header --- */}
        <div className="flex justify-between items-center mb-10 border-b border-gray-100 pb-8">
          <div className="w-1/3">
            <h1 className="text-3xl font-extrabold">
              <span className="block text-transparent bg-clip-text bg-gradient-to-r from-blue-800 to-blue-600 leading-tight uppercase">
                Settings & Profile
              </span>
            </h1>
          </div>
        ...
        {message && (
          <div className={`text-center p-3 rounded-lg mb-6 text-sm font-bold ${message.startsWith('Error') ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'}`}>
              {message}
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            <section className="bg-gray-50 p-6 rounded-xl border border-gray-200">
              <h2 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-4">Your Profile</h2>
              <div className="flex flex-col items-center gap-4">
                <img src={currentAvatar} alt="Avatar" className="w-24 h-24 rounded-full border-4 border-white shadow-md" />
                <div className="text-center">
                  <p className="text-xl font-black text-gray-800 uppercase leading-none">{currentUsername}</p>
                  <p className="text-xs font-bold text-blue-500 uppercase mt-1">{currentUserRole}</p>
                </div>
              </div>
              <div className="mt-6 grid grid-cols-4 gap-2">
                {avatarOptions.map((avatar, i) => (
                  <img key={i} src={avatar} onClick={() => handleAvatarSelect(avatar)} className={`w-full rounded-full cursor-pointer border-2 transition-all ${currentAvatar === avatar ? 'border-blue-600 scale-110' : 'border-transparent hover:scale-105'}`} />
                ))}
              </div>
              {hasProfileChanged && (
                <button onClick={handleProfileSave} className="w-full mt-6 bg-blue-800 text-white font-black py-3 rounded-lg flex items-center justify-center gap-2 hover:bg-black transition-all text-xs uppercase tracking-widest">
                  <Save size={16} /> Update Avatar
                </button>
              )}
            </section>
          </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            {currentUserRole === 'admin' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Local Contracts */}
                <section className="bg-gray-50 p-6 rounded-xl border border-gray-200">
                  <h2 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-4">Manage Contracts</h2>
                  <div className="flex gap-2 mb-4">
                    <input type="text" value={newContract} onChange={(e) => setNewContract(e.target.value)} className="flex-1 p-2 border-b-2 border-gray-200 outline-none focus:border-blue-600 text-sm bg-transparent" placeholder="New contract..." />
                    <button onClick={() => handleAddLocalOption('contract')} className="bg-blue-800 text-white p-2 rounded-lg hover:bg-black transition-all"><PlusCircle size={20}/></button>
                  </div>
                  <ul className="space-y-2 max-h-40 overflow-y-auto">
                    {contractOptions.map(opt => (
                      <li key={opt.id} className="flex justify-between items-center bg-white p-2 rounded border border-gray-100 text-sm font-bold">
                        {opt.name}
                        <button onClick={() => handleDeleteLocalOption(opt.id, 'contract')} className="text-gray-300 hover:text-red-500"><Trash2 size={16}/></button>
                      </li>
                    ))}
                  </ul>
                </section>
                ...
                {/* Local Cards */}
                <section className="bg-gray-50 p-6 rounded-xl border border-gray-200">
                  <h2 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-4">Manage Cards</h2>
                  <div className="flex gap-2 mb-4">
                    <input type="text" value={newCard} onChange={(e) => setNewCard(e.target.value)} className="flex-1 p-2 border-b-2 border-gray-200 outline-none focus:border-blue-600 text-sm bg-transparent" placeholder="New card..." />
                    <button onClick={() => handleAddLocalOption('card')} className="bg-blue-800 text-white p-2 rounded-lg hover:bg-black transition-all"><PlusCircle size={20}/></button>
                  </div>
                  <ul className="space-y-2 max-h-40 overflow-y-auto">
                    {creditCardOptions.map(opt => (
                      <li key={opt.id} className="flex justify-between items-center bg-white p-2 rounded border border-gray-100 text-sm font-bold">
                        {opt.name}
                        <button onClick={() => handleDeleteLocalOption(opt.id, 'card')} className="text-gray-300 hover:text-red-500"><Trash2 size={16}/></button>
                      </li>
                    ))}
                  </ul>
                </section>
            ...
            {currentUserRole === 'admin' && (
              <section className="bg-gray-50 p-6 rounded-xl border border-gray-200">
                <h2 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-4">User Directory</h2>
                <input type="text" placeholder="Filter users..." className="w-full mb-4 p-2 border-b-2 border-gray-200 outline-none focus:border-blue-600 bg-transparent text-sm" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-100">
                      {filteredUsers.map(user => (
                        <tr key={user.id} className="text-sm font-bold">
                          <td className="px-6 py-4">{user.username}</td>
                          <td className="px-6 py-4 capitalize text-blue-600">{user.role}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
        ...
        <div className="mt-10 flex justify-start">
          <button onClick={() => setCurrentPage('view')} className="flex items-center gap-2 text-gray-400 hover:text-black font-black text-xs uppercase tracking-widest transition-all">
