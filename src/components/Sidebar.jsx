...
import React from 'react';
import {
  Menu, Database, Briefcase, Settings, LogOut, Info,
  ClipboardList, Users, UserPlus, Building2, Truck
} from "lucide-react";
...
const SidebarItem = ({ icon, text, page, currentPage, setCurrentPage, isCollapsed }) => (
  <button
    ...
    className={`relative flex items-center w-full p-3 my-1 rounded-lg transition-colors duration-200 group cursor-pointer ${
      currentPage === page
        ? "bg-blue-900 text-white font-semibold shadow-lg" // Dark Blue Active State
        : "text-blue-900 hover:bg-blue-300" // Hover State
    }`}
  >
    {icon}
    <span className={`ml-4 transition-opacity duration-200 whitespace-nowrap ${isCollapsed ? "opacity-0 hidden" : "opacity-100"}`}>
      {text}
    </span>
    {isCollapsed && (
        <div className="absolute left-full rounded-md px-2 py-1 ml-6 bg-slate-900 text-white text-sm invisible opacity-0 -translate-x-3 transition-all group-hover:visible group-hover:opacity-100 group-hover:translate-x-0">
            {text}
        </div>
    )}
  </button>
...
export default function Sidebar({ currentPage, setCurrentPage, currentUserRole, handleLogout, sidebarOpen, setSidebarOpen }) {
  const navItems = [
    ...
    { page: 'employee-onboarding', label: 'Employee Information', icon: <UserPlus size={20} />, roles: ['user', 'admin', 'accountant'] },
    { page: 'customer', label: 'Customer Setup', icon: <Building2 size={20} />, roles: ['user', 'admin', 'accountant'] },
    { page: 'vendor', label: 'Vendor Setup', icon: <Truck size={20} />, roles: ['user', 'admin', 'accountant'] },
    // { page: 'accountant', label: 'Voucher Entries', icon: <Briefcase size={20} />, roles: ['admin', 'accountant'] },
    { page: 'user-profile', label: 'Settings & Profile', icon: <Settings size={20} />, roles: ['user', 'admin', 'accountant'] },
    { page: 'about', label: 'About', icon: <Info size={20} />, roles: ['user', 'admin', 'accountant'] },
  ];

  return (
    <div className={`fixed inset-y-0 left-0 bg-blue-200 text-slate-900 shadow-xl z-40 flex flex-col border-r border-blue-300 transition-all duration-300 ${sidebarOpen ? "w-64" : "w-20"}`}>
      <div className="flex flex-col flex-1 p-3 overflow-y-auto">
        <div className={`flex items-center mb-6 p-2 ${sidebarOpen ? "justify-start" : "justify-center"}`}>
          <button onClick={() => setSidebarOpen(!sidebarOpen)} className="p-1 rounded-lg hover:bg-blue-300 transition-colors">
            <Menu className="h-6 w-6 text-blue-900" />
          </button>
          <span className={`ml-3 text-lg font-black text-blue-950 tracking-tighter transition-all duration-200 ${sidebarOpen ? "opacity-100" : "opacity-0 hidden"}`}>
            Revolve MDM Portal
          </span>
        </div>
        ...
        <nav className="flex-grow">
          {navItems.map(item =>
            item.roles.includes(currentUserRole) && (
              <SidebarItem key={item.page} icon={item.icon} text={item.label} page={item.page} currentPage={currentPage} setCurrentPage={setCurrentPage} isCollapsed={!sidebarOpen} />
            )
          )}
        </nav>

        <div className="pt-2 mt-auto border-t border-blue-300 text-center py-4">
            <span className={`text-[10px] font-bold text-blue-800  tracking-widest transition-opacity duration-200 ${!sidebarOpen ? 'hidden' : 'opacity-100'}`}>
                Powered By Revolve
            </span>
        </div>
      </div>
