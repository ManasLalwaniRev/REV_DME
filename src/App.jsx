...
// import { MOCK_ENTRIES, MOCK_CONTRACT_OPTIONS, MOCK_CREDIT_CARD_OPTIONS } from './mockData';
...
//   const [dataEntries, setDataEntries] = useState(MOCK_ENTRIES);
//   const [contractOptions, setContractOptions] = useState(MOCK_CONTRACT_OPTIONS);
//   const [creditCardOptions, setCreditCardOptions] = useState(MOCK_CREDIT_CARD_OPTIONS);
//   const [isLoading, setIsLoading] = useState(false); // Set to false since data is local
...
// Specialized Form Components created for the demo
import ProjectSetupForm from '@/components/ProjectSetupForm.jsx';
import ProjectWorkforceForm from '@/components/ProjectWorkforceForm.jsx';
...
// All Mock Datasets from mockData.js
...
const App = () => {
  ...
  const [currentUserId] = useState(() => localStorage.getItem('luminaUserId'));
  const [currentUsername] = useState(() => localStorage.getItem('luminaUsername') || 'User');
  const [currentUserRole] = useState(() => localStorage.getItem('luminaUserRole') || 'admin');
  const [currentUserAvatar] = useState(() => localStorage.getItem('luminaUserAvatar'));
  ...
  const handleLoginSuccess = (userId, username, role, avatar) => {
    ...
    setIsLoggedIn(true);
    // Reload ensures all components re-read the demo auth state correctly
  ...
  const renderContent = () => {
    ...
    switch (currentPage) {
      // 1. Master Data Dashboard (Admin Master View)
      case 'view': 
        return (
          <ViewPage 
            {...commonProps} 
            allData={{
              expenses: MOCK.MOCK_ENTRIES || [],
              projectSetup: MOCK.MOCK_PROJECT_SETUP || [],
              workforce: MOCK.MOCK_WORKFORCE || [],
              onboarding: MOCK.MOCK_ONBOARDING || [],
              customers: MOCK.MOCK_CUSTOMERS || [],
              vendors: MOCK.MOCK_VENDORS || []
            }}
          />
        );
      
      // 2. Specialized Demo Entry Forms
      case 'project-setup': 
        return <ProjectSetupForm />;
      case 'project-workforce': 
        return <ProjectWorkforceForm />;
      ...
      // 3. Original Portal Pages
      case 'accountant':
        return (
          <AccountantPage 
            {...commonProps} 
            dataEntries={MOCK.MOCK_ENTRIES || []}
            userId={currentUserId} 
            userRole={currentUserRole} 
            fetchEntries={() => {}} 
          />
      ...
      default:
        return (
          <ViewPage 
            {...commonProps} 
            allData={{ expenses: MOCK.MOCK_ENTRIES || [] }}
          />
        );
    }
  };
  ...
  return (
    <div className="relative min-h-screen w-full bg-gray-100 flex overflow-hidden">
      {/* Yellow Themed Sidebar */}
      <Sidebar 
        currentPage={currentPage} 
        setCurrentPage={setCurrentPage} 
        currentUserRole={currentUserRole} 
      ...
      {/* Main Content Area with Layout Fixes:
          - 'ml-64/ml-20' provides space for the fixed sidebar.
          - 'max-w-full' and 'overflow-hidden' ensure the table scrollbar triggers 
            inside the view rather than pushing the whole screen wide.
      */}
      <main className={`transition-all duration-300 ease-in-out flex-grow ${sidebarOpen ? 'ml-64' : 'ml-20'} max-w-full overflow-hidden`}>
        {renderContent()}
      </main>
    </div>
  );
